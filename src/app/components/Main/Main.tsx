...
export function Main (props: IMainProps) {
  ...
  React.useEffect(() => { 
    window.localStorage.setItem("foodPageData", JSON.stringify(foodPageData)); 
  }, [])
  React.useEffect(() => { 
    const data = window.localStorage.getItem("foodPageData");
    if (data !== null) {
      setFoodPageData(JSON.parse(data));
